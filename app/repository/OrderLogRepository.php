...
class OrderLogRepository extends BaseRepository
{
     ...
     * 按订单 ID 分页查询订单日志
     */
    public function paginatedByOrder($orderId, int $page, int $pageSize)
    {
        return $this->query()
     ...
     * 获取订单最近一次催发货记录
     */
    public function getLastRemindByOrder(int $orderId): ?OrderLog
    {
        return $this->query()
            ->where('order_id', $orderId)
            ->where('change_type', 90)
            ->orderBy('created_at', 'desc')
            ->first();
    }
     ...
     * 按 app 获取最近的订单日志
     */
    public function listRecentByApp(int $appId, int $limit = 50)
    {
        $query = $this->query();
        if ($appId > 0) {
            $query->where('app_id', $appId);
